...
void reverse_list()
{
	NODE *left=NULL,*curr=head,*right;

	while(curr!=NULL)
	{
		right=curr->next; //present current node next becomes right
		curr->next=left;	//whoever is left node after reverse become next of of current
		left=curr;		//present curr node becomes left of next curr
		curr=right;		//present right becomes next right
	}
	head=left;
}
